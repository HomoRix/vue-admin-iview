...
export default {
  ...
  methods: {
    ...
    getData (params) {
      this.loading2 = true
      this.$api.getSuggestionSubmitListData(params).then(res => {
    ...
    remove (tagId, index) {
      let params = {}
      params.ids = tagId
      this.$api.deleteSuggestionSubmit(params).then(res => {
        let responseData = res // res.data
        if (!responseData) {
          return
        }
    ...
    saveBatch (suggestionSubmit) {
      ...
      if (this.currIndex !== -1) { // 编辑模式
        ...
        this.$api.updateSuggestionSubmit(suggestionSubmit).then(res => {
          let responseData = res // res.data
          if (!responseData) {
            return
          }
        ...
        this.$api.addSuggestionSubmit(suggestionSubmit).then(res => {
          let responseData = res // res.data
          if (!responseData) {
            return
          }
