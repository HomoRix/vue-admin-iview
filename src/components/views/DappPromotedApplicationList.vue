...
export default {
  ...
  methods: {
    ...
    getData (params) {
      this.loading2 = true
      this.$api.getDappPromotedApplicationListData(params).then(res => {
    ...
    remove (tagId, index) {
      let params = {}
      params.ids = tagId
      this.$api.deleteDappPromotedApplication(params).then(res => {
        let responseData = res // res.data
        if (!responseData) {
          return
        }
    ...
    saveBatch (dappPromotedApplication) {
      ...
      if (this.currIndex !== -1) { // 编辑模式
        ...
        this.$api.updateDappPromotedApplication(dappPromotedApplication).then(res => {
          let responseData = res // res.data
          if (!responseData) {
            return
          }
        ...
        this.$api.addDappPromotedApplication(dappPromotedApplication).then(res => {
          let responseData = res // res.data
          if (!responseData) {
            return
          }
