    ...
    <Modal v-model='editModal' v-if='DateReady' :style="{'z-index': 300}">
      ...
      <div style='text-align:center'>
        <Form :model='formItem' :label-width='80'>
          ...
          <Form-item label='用户名*'>
            <Input v-model='currDate.username' placeholder='请输入'/>
          </Form-item>
          <Form-item label='邮箱*'>
            <Input v-model='currDate.email' placeholder='请输入'/>
          </Form-item>
          <Form-item label='输入密码*'>
            <Input v-model='currDate.password' placeholder='请输入'/>
          </Form-item>
...
// import { getAdminListData, deleteAdmin, updateAdmin, addAdmin } from '@/api/data'
...
export default {
  ...
  methods: {
    ...
    getData (params) {
      this.loading2 = true
      this.$api.getAdminListData(params).then(res => {
    ...
    show (index) {
      ...
      this.$Modal.info({
        title: '详情',
        content: `用户名：${this.listData[index].username}<br>姓名：${
          this.listData[index].name
        }<br>邮箱：${this.listData[index].email}`
      })
    },
    remove (tagId, index) {
      let params = {}
    ...
    saveBatch (admin) {
      ...
      if (this.currIndex !== -1) { // 编辑模式
        ...
        this.$api.addAdmin(admin).then(res => {
          let responseData = res // res.data
          if (!responseData) {
            return
          }
