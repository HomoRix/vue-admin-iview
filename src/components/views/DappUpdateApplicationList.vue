...
export default {
  ...
  methods: {
    ...
    getData (params) {
      this.loading2 = true
      this.$api.getDappUpdateApplicationListData(params).then(res => {
    ...
    remove (tagId, index) {
      let params = {}
      params.ids = tagId
      this.$api.deleteDappUpdateApplication(params).then(res => {
        let responseData = res // res.data
        if (!responseData) {
          return
        }
    ...
    saveBatch (dappUpdateApplication) {
      ...
      if (this.currIndex !== -1) { // 编辑模式
        ...
        this.$api.updateDappUpdateApplication(dappUpdateApplication).then(res => {
          let responseData = res // res.data
          if (!responseData) {
            return
          }
        ...
        this.$api.addDappUpdateApplication(dappUpdateApplication).then(res => {
          let responseData = res // res.data
          if (!responseData) {
            return
          }
