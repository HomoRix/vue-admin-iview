...
export default {
  ...
  methods: {
    ...
    getData (params) {
      this.loading2 = true
      this.$api.getSubscriberListData(params).then(res => {
    ...
    remove (tagId, index) {
      let params = {}
      params.ids = tagId
      this.$api.deleteSubscriber(params).then(res => {
        let responseData = res // res.data
        if (!responseData) {
          return
        }
    ...
    saveBatch (subscriber) {
      ...
      if (this.currIndex !== -1) { // 编辑模式
        ...
        this.$api.updateSubscriber(subscriber).then(res => {
          let responseData = res // res.data
          if (!responseData) {
            return
          }
        ...
        this.$api.addSubscriber(subscriber).then(res => {
          let responseData = res // res.data
          if (!responseData) {
            return
          }
